...
const generateContractEditedEmailHtml = ({ seller, buyer, requestTitle, price, timeline, policy, schedule, logoCid }) => {
  const scheduleRows = schedule.map(item => `
    <tr>
      <td style="padding: 8px; border: 1px solid #ccc;">${item.date}</td>
      <td style="padding: 8px; border: 1px solid #ccc;">$${item.amount}</td>
  ...
  return `
    ...
    <body style="font-family: Arial, sans-serif; background-color: #f9f9f9; margin: 0; padding: 0;">
      <table width="100%" cellpadding="0" cellspacing="0" style="max-width: 600px; margin: auto; background-color: #ffffff; border-radius: 8px; overflow: hidden;">
        ...
        <tr>
          <td style="padding: 20px;">
            <h2 style="color: #124662;">Contract Updated</h2>
            <p>Dear <strong>${seller.first_name} ${seller.last_name}</strong>,</p>
            <p>
              Buyer <strong>${buyer.first_name} ${buyer.last_name}</strong> has updated the contract for the procurement request titled:
              <strong>"${requestTitle}"</strong>.
            </p>

            <p><strong>Updated Contract Details:</strong></p>
            <ul>
              <li><strong>Price:</strong> $${price}</li>
              <li><strong>Timeline:</strong> ${timeline}</li>
              <li><strong>Payment Policy:</strong> ${policy}</li>
            ...
            <p><strong>Updated Payment Schedule:</strong></p>
            <table width="100%" cellpadding="0" cellspacing="0" style="border-collapse: collapse;">
              <thead>
                <tr style="background-color: #f2f2f2;">
                  <th style="padding: 8px; border: 1px solid #ccc;">Date</th>
            ...
            <p style="margin-top: 20px;">You can review the updated contract by logging into your dashboard.</p>
...
module.exports = {
    ...
    generateContractEditedEmailHtml
};