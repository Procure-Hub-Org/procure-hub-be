  ...
  updateProcurementRequestStatus: async (req, res) => {
    try {
      ...
      if (procurementRequest.status === "draft" && status != "active") {
        return res
          .status(400)
          .json({ message: "Status from draft can only be changed to active" });
      }
      if (
    ...
    try {
      ...
      if (status === "closed" || status === "awarded") {
        return res
          ...
          .json({
            message:
              "Cannot set status to closed or awarded through this route",
          });
      }
      if (procurementRequest.status != "draft") {
        return res
          ...
          .json({
            message:
              "Cannot update procurement with status active, update or awarded.",
          });
      }
      ...
      if (status === "active") {
        if (
          ...
          return res
            ...
            .json({
              message:
                "All fields and at least one item are required when setting status to active",
            });
        }
      }
